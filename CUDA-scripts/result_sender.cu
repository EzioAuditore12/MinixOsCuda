...
void send_results(double time, std::string mode) {
    std::string path_to_send = "/mnt/d/Users/MinixOsCuda/src/stats/";
    std::string file_name = "result.ts";
    std::string filepath = path_to_send + file_name;
    ...
    if (infile.is_open()) {
        ...
        while (std::getline(infile, line)) {
            // Parse CPU time
            if (line.find("const cpu_time") != std::string::npos) {
                ...
                if (equal_pos != std::string::npos) {
                    ...
                    value_str.erase(std::remove_if(value_str.begin(), value_str.end(), 
                                   ...
                                   value_str.end());
                    try {
                        cpu_time = std::stod(value_str);
                    } catch (...) {
                        std::cerr << "Failed to parse CPU time: " << value_str << std::endl;
                    }
                }
            }
            // Parse GPU time
            else if (line.find("const gpu_time") != std::string::npos) {
                size_t equal_pos = line.find("=");
                if (equal_pos != std::string::npos) {
                    std::string value_str = line.substr(equal_pos + 1);
                    // Remove non-numeric characters
                    value_str.erase(std::remove_if(value_str.begin(), value_str.end(), 
                                   ...
                                   value_str.end());
                    try {
                        gpu_time = std::stod(value_str);
                    } catch (...) {
                        std::cerr << "Failed to parse GPU time: " << value_str << std::endl;
                    }
                }
            }
        }
    ...
    // Update only the time for the specified mode
    if (mode == "gpu") {
        gpu_time = time;
    } else if (mode == "cpu") {
        cpu_time = time;
    ...
    if (result_file.is_open()) {
        result_file << "const gpu_time: number = " << gpu_time << ";\n"
                    << "const cpu_time: number = " << cpu_time << ";\n"
                    << "export { gpu_time, cpu_time };";
        result_file.close();
        std::cout << "Results saved to file. Updated " << mode << " time to " << time << " s" << std::endl;
    } else {
